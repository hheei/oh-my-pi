...
function span(
	...
	for (const match of line.matchAll(pattern)) {
		...
		if (
			(action === "left" && start < col && col <= end) ||
			(action === "right" && start <= col && col < end) ||
			(action === "backspace" && start < col && col <= end) ||
			(action === "delete" && start <= col && col < end)
		)
			return { start, end };
	}
}

export function createPercentAtomicEditor(
	editor: CustomEditor,
	commands: () => readonly SlashCommandInfo[],
	enabled: () => boolean,
): CustomEditor {
	...
	editor.handleInput = (data: string) => {
		...
		const action: Action | undefined = matchesKey(data, "left")
			? "left"
		...
		const count =
			action === "left" ? cursor.col - found.start : found.end - cursor.col;
