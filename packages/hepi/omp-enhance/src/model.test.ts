...
describe("percent skill references", () => {
	test("expands known skill references while leaving unknown text unchanged", () => {
		expect(
			expandPercentReferences("Use %reader, then %missing.", commands),
		).toBe("Use /skills/reader/SKILL.md, then %missing.");
	});
	test("extracts only a percent token at a word boundary", () => {
		expect(extractPercentToken(["Use %rea"], 0, 8)).toEqual({
			query: "rea",
			prefix: "%rea",
		});
	...
	test("suggestions include skills only and honor limit", () => {
		expect(
			getPercentSuggestions(commands, "", 1).map((item) => item.value),
		).toEqual(["%reader"]);
	});
	test("normalizes persisted settings at trust boundary", () => {
		expect(normalizeConfig({ enabled: false, maxSuggestions: 999 })).toEqual({
			enabled: false,
			maxSuggestions: 50,
		});
	});
	test("wrapper keeps optional host provider methods", async () => {
		...
		const provider = createPercentProvider(
			current,
			() => commands,
			() => ({ enabled: true, maxSuggestions: 50 }),
		);
		...
		expect(provider.trySyncSlashCompletion?.("/h")).toEqual({
			items: [{ value: "/help", label: "help" }],
			prefix: "/",
		});
		expect(provider.trySyncInlineReplace?.("->")).toEqual({
			replaceLen: 2,
			insert: "=>",
		});
		expect(
			await provider.getForceFileSuggestions?.(["Use %rea"], 0, 8),
		).toBeNull();
	});
});
