...
export const DEFAULT_CONFIG: PercentConfig = {
	enabled: true,
...
export function extractPercentToken(
...
export function getPercentSuggestions(
	commands: readonly SlashCommandInfo[],
	query: string,
	limit: number,
): AutocompleteItem[] {
	return skills(commands)
		.filter((c) => nameOf(c).toLowerCase().startsWith(query.toLowerCase()))
		...
		.map((command) => ({
			...
			description:
				`${command.location ? `${command.location[0].toUpperCase()}${command.location.slice(1)} - ` : "Skill - "}${command.description?.trim() ?? ""}`.trim(),
		}));
}
export function expandPercentReferences(
	text: string,
	commands: readonly SlashCommandInfo[],
): string | undefined {
	const paths = new Map<string, string>();
	for (const command of skills(commands)) {
		if (command.path) paths.set(nameOf(command), command.path);
	}
	let changed = false;
...
export function createPercentProvider(
	...
	return {
		...
		async getSuggestions(lines, line, col, signal) {
			...
			const items = getPercentSuggestions(
				commands(),
				token.query,
				config().maxSuggestions,
			);
			return items.length
				? { prefix: token.prefix, items }
				: current.getSuggestions(lines, line, col, signal);
		},
		...
		shouldTriggerFileCompletion(lines, line, col) {
			return percentToken(lines, line, col)
				? false
		...
		getForceFileSuggestions: getForceFileSuggestions
			? async (lines, line, col, signal) =>
					percentToken(lines, line, col)
						? null
						: getForceFileSuggestions(lines, line, col, signal)
			: undefined,
		getInlineHint: getInlineHint
			? (lines, line, col) =>
					percentToken(lines, line, col)
						? null
						: getInlineHint(lines, line, col)
			: undefined,
