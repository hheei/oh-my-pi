...
} from "../../../src/core/tools/unwrap-imitated-reduced-args";
...
describe("imitated reduced argument revalidation", () => {
	for (const arm of cases) {
		test(`${arm.name} accepts only schema-valid decoded fields`, () => {
			...
			expect(unwrapImitatedReducedArgs(validOuter, arm.primary, arm.schema)).toEqual(arm.valid);
			...
			for (const invalid of [
				...
				expect(() => unwrapImitatedReducedArgs(outer, arm.primary, arm.schema)).not.toThrow();
