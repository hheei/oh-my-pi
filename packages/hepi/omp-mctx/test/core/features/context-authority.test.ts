...
describe("memory authority protocol", () => {
	...
	test("foreign-store note ids allocate a fresh row without clobbering a local collision", () => {
		...
		expect(database.prepare("SELECT id, content, session_id FROM notes ORDER BY id").all()).toEqual(
			[
	...
	test("mapping feed rows round-trip into the verification side table", () => {
		...
		const verification = getMemoryVerifications(database, [contextMemory.id]).get(contextMemory.id);
		...
		expect(getMemoryVerifications(database, [contextMemory.id]).has(contextMemory.id)).toBe(false);
	});
	...
	test("module checksum mismatch aborts, removes the marker, and restores TS writes", async () => {
		...
		await expect(
			prepareAuthority({
				...
				domains: ["memories"],
				module,
				seedPages: async () => [{ source_row_id: 1, snapshot: { id: 1, project_path: "/repo" } }],
			}),
	...
	test("mirror updates delete stale vectors and translate references atomically", () => {
		...
		withPrivilegedWriter(database, () => {
			...
			database
				.prepare("INSERT INTO memory_embeddings(memory_id, embedding, model_id) VALUES (1, ?, ?)")
		...
		expect(
			database.prepare("SELECT COUNT(*) AS count FROM memory_embeddings WHERE memory_id = 1").get(),
		).toEqual({ count: 0 });
		withPrivilegedWriter(database, () => {
			database
				.prepare("INSERT INTO memory_embeddings(memory_id, embedding, model_id) VALUES (1, ?, ?)")
		...
		expect(database.prepare("SELECT COUNT(*) AS count FROM memories WHERE id = 1").get()).toEqual({
			count: 0,
		});
		expect(
			database.prepare("SELECT COUNT(*) AS count FROM memory_embeddings WHERE memory_id = 1").get(),
	...
	test("mirror-back keeps same content in separate project rows", () => {
		...
		expect(
			database
				.prepare("SELECT id, project_path, category, normalized_hash FROM memories ORDER BY id")
				.all(),
		).toEqual([
			{
				id: 9395,
	...
	test("mirror-back pins row-id adoption to the local context store UUID", () => {
		...
		expect(
			database.prepare("SELECT id, project_path, normalized_hash FROM memories ORDER BY id").all(),
		).toEqual([
	...
	test.skip("schema-57 mirror upgrades resnapshot before either tombstone order", async () => {
		...
		for (const scenario of scenarios) {
			...
			const calls: Array<{ live_only?: boolean | undefined }> = [];
	...
	test.skip("DRAINING recovery resnapshots schema-57 memory identities before tombstones", async () => {
		...
		for (const interruptedStatus of ["pending_check", "resnapshotting"] as const) {
			...
			const calls: Array<{ live_only?: boolean | undefined; cursor: number }> = [];
	...
	test("stages paged live resnapshots and swaps only after the final page", async () => {
		...
		expect(
			database
				.prepare("SELECT module_project FROM mirror_live_memory_rows ORDER BY module_row_id")
				.all(),
		).toEqual([
	...
	test("a stale paged resnapshot cannot replace a newer completed generation", async () => {
		...
		try {
			...
			expect(first.prepare("SELECT status, generation FROM mirror_resnapshot_state").get()).toEqual(
				{
					status: "complete",
					generation: expect.any(String),
				},
			);
			expect(first.prepare("SELECT COUNT(*) AS count FROM mirror_live_staging").get()).toEqual({
				count: 0,
			});
		} finally {
			first.close();
	...
	test("does not hold a SQLite transaction while module transport is delayed", async () => {
		...
		releaseBegin?.();
		await preparation;
		expect(database.prepare("SELECT COUNT(*) AS count FROM unrelated_writer_probe").get()).toEqual({
			count: 1,
		});
	});
	...
	test("foreign archived expired and unshareable rows stay hidden on the id search path", () => {
		...
		expect(idPath?.map((hit) => hit.content).sort()).toEqual(["foreign visible", "own archived"]);
	});
