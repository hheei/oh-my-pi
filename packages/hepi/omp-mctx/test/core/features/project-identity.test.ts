...
describe("project identity", () => {
	...
	it("uses the no-git fast path without invoking git", () => {
		...
		__setProjectIdentityTestHooks({
			execFileSync: execMock as unknown as unknown as typeof execFileSync,
		});
	...
	it("classifies dubious ownership and falls back until the cooldown expires", () => {
		...
		const strictError = expectProjectIdentityError(() => resolveProjectIdentityStrict(directory));
	...
	it("still propagates permission_denied git failures", () => {
		...
		__setProjectIdentityTestHooks({
			...
			}) as unknown as unknown as typeof execFileSync,
		});
