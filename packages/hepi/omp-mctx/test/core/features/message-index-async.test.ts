...
function pagedReader(
	...
	return Object.assign((_sessionId: string) => messages, {
		...
		readPage: (_sessionId: string, afterOrdinal: number, limit: number, finalWatermark: number) => {
			onPage?.(afterOrdinal);
			return messages
...
describe("message-index-async", () => {
	...
	it("replays the same source revision without replacing its FTS row", async () => {
		...
		await wait(140);
		const before = db
			.prepare("SELECT rowid FROM message_history_fts WHERE session_id = ? AND message_id = ?")
