...
describe("mural cue storage", () => {
	...
	test("writes only cue cache columns for an authority-managed memory", () => {
		...
		try {
			...
			expect(() =>
				db.prepare("UPDATE memories SET content = ? WHERE id = ?").run("forbidden edit", memory.id),
...
describe("applyCues (per-cue validation, skip-not-reject, hash-race)", () => {
	...
	test("stores the SELECTION-time content hash so an edit mid-run yields a stale (excluded) cue", () => {
		...
		try {
			...
			updateMemoryContent(db, memory.id, "edited content", computeNormalizedHash("edited content"));
	...
	test.each([
		...
		try {
			...
			expect(() => applyCues(cueArgs(db, "git:p"), chunk, manifestFor(memory.id))).toThrow(error);
