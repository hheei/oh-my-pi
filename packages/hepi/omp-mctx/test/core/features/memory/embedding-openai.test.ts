...
} from "../../../../src/core/features/memory/embedding-openai";
...
describe("provider modelId matches canonical identity (write/read must agree)", () => {
	...
	for (const c of cases) {
		test(c.name, () => {
			if (c.config.provider !== "openai-compatible") {
				throw new Error(`expected openai-compatible config in ${c.name}`);
			}
			const provider = new OpenAICompatibleEmbeddingProvider({
				endpoint: c.config.endpoint,
				model: c.config.model,
				apiKey: c.config.api_key,
				inputType: c.config.input_type,
				truncate: c.config.truncate,
			});
...
describe("embeddingModelsMatch token-boundary semantics", () => {
	...
	test("version-expansion suffix on a boundary matches", () => {
		expect(embeddingModelsMatch("text-embedding-3-small-v1", "text-embedding-3-small")).toBe(true);
	});
	test("vendor-prefix trim on a boundary matches (either direction)", () => {
		expect(embeddingModelsMatch("openai/text-embedding-3-small", "text-embedding-3-small")).toBe(
			true,
		);
		expect(embeddingModelsMatch("text-embedding-3-small", "openai/text-embedding-3-small")).toBe(
			true,
		);
	});
	...
	test("REJECTS a non-boundary prefix collision (small vs smaller)", () => {
		expect(embeddingModelsMatch("text-embedding-3-smallish", "text-embedding-3-small")).toBe(false);
	});
...
describe("OpenAICompatibleEmbeddingProvider model-substitution guard", () => {
	...
	test("accepts vectors when the served model matches exactly", async () => {
		fetchSpy.mockImplementation((async () => modelResponse("qwen3-embedding-4b-dwq")) as FetchLike);
		const provider = new OpenAICompatibleEmbeddingProvider({
			endpoint: "http://127.0.0.1:65535",
			model: "qwen3-embedding-4b-dwq",
		});
