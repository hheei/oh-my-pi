...
function getStoredProjectPath(db: Database, sessionId: string): string | null {
	const row = db
		.prepare("SELECT project_path FROM session_projects WHERE session_id = ? AND harness = 'pi'")
...
describe("runSessionProjectBackfill", () => {
	...
	it("is a no-op after the harness backfill is marked completed", async () => {
		...
		const second = await runSessionProjectBackfill(db, [{ sessionId: "ses-second", directory }], {
			resolveIdentity: () => {
				resolverCalls += 1;
				return "git:second";
			},
	...
	it("blocks on an active lease and reclaims an expired lease", async () => {
		...
		const blocked = await runSessionProjectBackfill(db, [{ sessionId: "ses-lease", directory }], {
		...
		const reclaimed = await runSessionProjectBackfill(db, [{ sessionId: "ses-lease", directory }], {
			resolveIdentity: () => "git:lease",
			now: () => 5200,
		});
