...
function rowCount(): number {
	return (
		db
			.prepare(
				"SELECT COUNT(*) AS c FROM transform_decisions WHERE session_id = 'ses-1' AND harness = 'pi'",
			)
			.get() as { c: number }
	).c;
}
...
describe("transform_decisions retention cap", () => {
	...
	it("prunes to the newest cap rows per (session,harness)", () => {
		...
		const oldest = db.prepare("SELECT 1 FROM transform_decisions WHERE message_id = 'msg-0'").get();
		const newest = db
