...
import {
	...
	nextOccurrence,
	parseCron,
} from "../../../../src/core/features/dreamer/cron";
...
describe("parseCron — valid forms", () => {
	...
	it("accepts lists", () => {
		expect([...parsed("0,15,30,45 * * * *").minute].sort((a, b) => a - b)).toEqual([0, 15, 30, 45]);
	});
	it("accepts ranges", () => {
	...
	it("accepts combined list+range+step", () => {
		expect([...parsed("0 0,6-8,*/12 * * *").hour].sort((a, b) => a - b)).toEqual([0, 6, 7, 8, 12]);
	});
});
