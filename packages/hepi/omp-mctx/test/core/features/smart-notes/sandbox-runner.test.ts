...
import {
	...
	type SmartNoteCapabilityApi,
} from "../../../../src/core/features/smart-notes/capabilities";
				...
				capabilityFactory: (signal) => ({
					...
					httpGet: () =>
						new Promise((_resolve, reject) => {
							const abort = () => reject(new SmartNoteNetworkError("SMART_NOTE_NETWORK: aborted"));
							if (signal.aborted) {
								abort();
								return;
							}
		...
		const followup = (await Promise.race([
			...
			new Promise<never>((_, reject) =>
				setTimeout(
					() =>
						reject(
							new Error("follow-up sandbox run stayed blocked behind the timed-out host call"),
						),
					500,
				),
			),
