...
describe("project embedding registry", () => {
	...
	it("default local config (no local_dtype) keeps the golden identity — no re-embed on upgrade (#259)", () => {
		...
		expect(noDtype.providerIdentity).toBe("embedding-provider:c447205ebd551e83d18c4fd5fd8fc357");
	});
	...
	it("drainCommitBacklogForProject embeds pre-indexed commits with no new git log work", async () => {
		_setTestProviderFactoryForProject(
			(config) => new FakeEmbeddingProvider(config.provider === "local" ? config.model : "off"),
		);
		const db = useTempDb();
		...
		const drained = await drainCommitBacklogForProject(db, projectIdentity, Date.now() + 60_000);
	...
	it("skips stale sweep results when a memory changes before the batch save", async () => {
		...
		expect(loadAllEmbeddings(db, projectIdentity, currentModelId(projectIdentity)).size).toBe(0);
	});
	...
	it("keeps memory, commit, and chunk embeddings coexisting per model", () => {
		...
		expect(
			Array.from(loadAllEmbeddings(db, projectIdentity, first.modelId).get(memory.id)!.embedding),
		).toEqual([1, 0]);
		expect(
			Array.from(loadAllEmbeddings(db, projectIdentity, second.modelId).get(memory.id)!.embedding),
		).toEqual([0, 2]);
		expect(
			countRows(
				db,
	...
	it("garbage-collects only stale inactive embedding identities after the grace window", () => {
		...
		expect(
			loadCompartmentChunkEmbeddingsForSearch(db, "ses-gc", projectIdentity, second.chunkModelId),
		).toHaveLength(1);
	...
	it("getShadowBackfillStopReason returns drained after a successful shadow backfill", async () => {
		_setTestProviderFactoryForProject(
			(config) => new FakeEmbeddingProvider(config.provider === "local" ? config.model : "shadow"),
		);
		const db = useTempDb();
