...
describe("createScheduler", () => {
	...
	it("uses a custom execute threshold", () => {
		...
		expect(scheduler.shouldExecute(sessionMeta, createContextUsage(69), BASE_TIME)).toBe("defer");
		expect(scheduler.shouldExecute(sessionMeta, createContextUsage(70), BASE_TIME)).toBe("execute");
	});
