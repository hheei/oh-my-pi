...
import {
	createTagger,
	type Tagger,
	type ToolTagAccounting,
} from "../../../src/core/features/tagger";
import type { ToolDropResult } from "../../../src/core/hooks/tool-drop-target";
...
import type {
	...
	TranscriptPart,
	TranscriptPartKind,
} from "../../../src/core/shared/transcript";
...
class FakeTagger implements Tagger {
	...
	assignTag(..._args: Parameters<Tagger["assignTag"]>): number {
		return this.nextTag++;
	}

	getTag(..._args: Parameters<Tagger["getTag"]>): number | undefined {
		return undefined;
	}

	assignToolTag(...args: Parameters<Tagger["assignToolTag"]>): number {
		const [, callId, ownerMsgId, byteSize] = args;
	...
	getToolTag(...args: Parameters<Tagger["getToolTag"]>): number | undefined {
		const [, callId, ownerMsgId] = args;
		return this.toolTags.get(`${ownerMsgId}\0${callId}`);
	}

	getToolTagAccounting(
		...args: Parameters<Tagger["getToolTagAccounting"]>
	): ToolTagAccounting | undefined {
		const [, _callId, _ownerMsgId] = args;
		const tag = this.getToolTag(...args);
		return tag === undefined ? undefined : this.toolAccounting.get(tag);
	}

	setToolTagAccounting(...args: Parameters<Tagger["setToolTagAccounting"]>): void {
		const [, tagNumber, accounting] = args;
		this.toolAccounting.set(tagNumber, accounting);
	}

	bindTag(..._args: Parameters<Tagger["bindTag"]>): void {}

	unbindTag(..._args: Parameters<Tagger["unbindTag"]>): void {}

	bindToolTag(...args: Parameters<Tagger["bindToolTag"]>): void {
		const [, callId, ownerMsgId, tagNumber] = args;
		this.toolTags.set(`${ownerMsgId}\0${callId}`, tagNumber);
	}

	unbindToolTag(..._args: Parameters<Tagger["unbindToolTag"]>): void {}

	getAssignments(..._args: Parameters<Tagger["getAssignments"]>): ReadonlyMap<string, number> {
		return this.toolTags;
	}

	resetCounter(..._args: Parameters<Tagger["resetCounter"]>): void {
		this.nextTag = 1;
	}

	getCounter(..._args: Parameters<Tagger["getCounter"]>): number {
		return this.nextTag - 1;
	}

	initFromDb(..._args: Parameters<Tagger["initFromDb"]>): void {}

	cleanup(..._args: Parameters<Tagger["cleanup"]>): void {}
}
...
describe("tagTranscript tool aggregation", () => {
	...
	it("truncates assistant tool_use parts via text fallback when setToolOutput asserts", () => {
		...
		let result: ToolDropResult | undefined;
		expect(() => {
	...
	it("reuses owner-scoped identities without collapsing duplicate callIds", () => {
		...
		const reused = tagTranscript(
			...
			{
				reuseMessageIds: new Set(["assistant-old", "user-old", "assistant-new", "user-new"]),
			},
		);

		const oldTag = tagger.getToolTag("session-duplicate-reuse", "duplicate-call", "assistant-old");
		const newTag = tagger.getToolTag("session-duplicate-reuse", "duplicate-call", "assistant-new");
	...
	it("persists the same grown folded-result accounting with reuse or full derivation", () => {
		...
		const runScenario = (reuse: boolean): { before: AccountingRow; after: AccountingRow } => {
			...
			tagTranscript(sessionId, makeTranscript("larger result ".repeat(80)), nextPassTagger, db, {
				reuseMessageIds: reuse ? new Set(["assistant-owner", "user-fold-target"]) : undefined,
			});
