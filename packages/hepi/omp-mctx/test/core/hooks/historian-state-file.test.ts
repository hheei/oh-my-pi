...
describe("maybeWriteHistorianStateFile", () => {
	...
	test("returns undefined exactly at threshold (only larger triggers offload)", () => {
		...
		expect(maybeWriteHistorianStateFile("ses_abc", atThreshold, tempProjectDir)).toBeUndefined();
	});

	test("writes large state to <project>/.pi/magic-context/historian/", () => {
		// Keep Pi's transient state inside the project boundary while using a
		// namespace that cannot collide with the upstream CortexKit extension.
		...
		expect(stateFile!).toContain(path.join(tempProjectDir, ".pi", "magic-context", "historian"));
		expect(stateFile!).toContain("state-ses_abc-");
		expect(stateFile!).toMatch(/\.xml$/);
		expect(existsSync(stateFile!)).toBe(true);
		expect(readFileSync(stateFile!, "utf8")).toBe(big);
		// The transient dump dir is git-ignored via a fenced .pi/.gitignore.
		const gi = path.join(tempProjectDir, ".pi", ".gitignore");
		expect(existsSync(gi)).toBe(true);
		expect(readFileSync(gi, "utf8")).toContain("magic-context/");
	});

	test("creates .pi/magic-context/historian/ recursively on fresh project", () => {
		// Fresh projects have no .pi/ subtree at all. The helper must
		// mkdir -p so it works without any prior setup.
		expect(existsSync(path.join(tempProjectDir, ".pi"))).toBe(false);
		...
		expect(stateFile).toBeDefined();
		expect(existsSync(path.join(tempProjectDir, ".pi", "magic-context", "historian"))).toBe(true);
	});
