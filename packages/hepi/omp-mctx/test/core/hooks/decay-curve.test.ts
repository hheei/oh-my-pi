...
describe("decay-curve — budget pressure", () => {
	...
	it("two-pass tightens an overshooting tight budget", () => {
		...
		const cost1 = comps.reduce(
			(s, c) => s + TIER_COST[tier(c.index, c.importance, computeBudgetPressure(comps, budget))],
			0,
		);
		const cost2 = comps.reduce(
			(s, c) =>
				s + TIER_COST[tier(c.index, c.importance, computeBudgetPressureTwoPass(comps, budget))],
			0,
		);
