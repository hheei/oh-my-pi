...
describe("parseCompartmentOutput with localized prose", () => {
	...
	it("extracts the same structural shape from Turkish prose", () => {
		...
		expect(
			shape.factCategories.every((category) =>
				(FACT_CATEGORIES as readonly string[]).includes(category),
			),
		).toBe(true);
		expect(
			shape.eventKinds.every((kind) => (EVENT_KINDS as readonly string[]).includes(kind)),
		).toBe(true);
	});
