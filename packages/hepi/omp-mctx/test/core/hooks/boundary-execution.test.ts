...
describe("applyMidTurnDeferral", () => {
	it("implements the boundary-execution decision table", () => {
		...
		expect(applyMidTurnDeferral({ base: "execute", bypassReason: "none", midTurn: true })).toEqual({
			midTurnAdjustedSchedulerDecision: "defer",
			sideEffect: "set-flag",
		});

		expect(applyMidTurnDeferral({ base: "execute", bypassReason: "none", midTurn: false })).toEqual(
			{ midTurnAdjustedSchedulerDecision: "execute", sideEffect: "none" },
		);

		expect(applyMidTurnDeferral({ base: "defer", bypassReason: "none", midTurn: true })).toEqual({
			midTurnAdjustedSchedulerDecision: "defer",
			sideEffect: "none",
		});
	});
