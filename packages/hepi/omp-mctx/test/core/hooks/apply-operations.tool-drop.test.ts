...
function asToolPart(part: unknown): ToolPartLike | undefined {
	if (part === null || typeof part !== "object") return undefined;
	return part as ToolPartLike;
}

function findCallPart(
	messages: MessageLike[],
	callId: string,
	type?: string,
): ToolPartLike | undefined {
	for (const message of messages) {
		for (const part of message.parts) {
			const candidate = asToolPart(part);
			if (candidate?.callID !== callId) continue;
			if (type !== undefined && candidate.type !== type) continue;
			return candidate;
		}
	}
	return undefined;
...
describe("apply operations for tool drops", () => {
	...
	it("keeps a skeleton (canonical [dropped §N§] output) for drops within the recent tool window", () => {
		...
		const toolPart = findCallPart(messages, "call-1", "tool");
	...
	it("edit_marker: preserves filePath + region hint, freezes mode, replays byte-identically", () => {
		...
		const editPart = findCallPart(messages, "call-edit");
		// filePath preserved verbatim; diff clamped; output → canonical sentinel.
		const oldString = editPart?.state?.input?.oldString;
		expect(editPart?.state?.input?.filePath).toBe("/Users/me/proj/spec.md");
		expect(typeof oldString === "string" && oldString.endsWith("...[truncated]")).toBe(true);
		...
		const replayed = findCallPart(replayMessages, "call-edit");
	...
	it("applies flushed dropped status to tool call slices", () => {
		...
		const toolPart = findCallPart(messages, "call-2", "tool");
		expect(toolPart).toBeDefined();
		expect(toolPart?.state?.output).toBe(`[dropped \u00a7${toolTagId}\u00a7]`);
	});
	...
	describe("role-aware message drops (canonical [dropped §N§])", () => {
		...
		it("keeps using [dropped §N§] full drop for assistant messages (no regression)", () => {
			...
			expect(didMutate).toBe(true);
			expect(messages[1]?.parts).toEqual([{ type: "text", text: `[dropped §${asstMsgTagId}§]` }]);
		});
