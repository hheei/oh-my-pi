...
import { getTagsBySession, insertTag, saveSourceContent } from "../../../src/core/features/storage";
...
describe("applyCavemanCleanup", () => {
	...
	test("no-op when no eligible tags", () => {
		...
		expect(result.compressedToLite + result.compressedToFull + result.compressedToUltra).toBe(0);
	});
