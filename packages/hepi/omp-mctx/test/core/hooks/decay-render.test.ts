...
describe("decay-render", () => {
	it("excludes legacy rows from v2 pressure and age indexing", () => {
		...
		const compartments = [v2, ...Array.from({ length: 80 }, (_, i) => legacyCompartment(i + 2))];
	...
	it("compresses same-day, same-month, and full date ranges", () => {
		...
		expect(
			renderCompartmentAtTier(
				{ ...base, startDate: "2026-06-08", endDate: "2026-06-08" },
				1,
			).startsWith("## 1-2 · 2026-06-08 · dated"),
		).toBe(true);
		expect(
			renderCompartmentAtTier(
				{ ...base, startDate: "2026-06-08", endDate: "2026-06-09" },
				1,
			).startsWith("## 1-2 · 2026-06-08→09 · dated"),
		).toBe(true);
		expect(
			renderCompartmentAtTier(
				{ ...base, startDate: "2026-06-08", endDate: "2026-07-02" },
				1,
			).startsWith("## 1-2 · 2026-06-08→2026-07-02 · dated"),
		).toBe(true);
	});
	...
	it("matches the previous guard's tiers and bytes across production-shape budgets", () => {
		...
		for (const budget of budgets) {
			...
			expect(tiersFromRenderedBody(rendered, compartments.length), `${budget.name} tiers`).toEqual(
				previous.tiers,
			);
