...
import { type PiPrefixTool, resolvePiContextUsage } from "#host/pi-ext-shim";
...
function renderStatusText(ctx: ExtensionContext, db: ContextDatabase, sessionId: string): string {
	...
	const usageOptions = {
		...
		...(typeof ctx.model?.contextWindow === "number" ? { contextWindow: ctx.model.contextWindow } : {}),
		...(liveReady
			? {}
			: {
...
function readSystemPrompt(ctx: ExtensionContext): string | undefined {
	try {
		const prompt = typeof ctx.getSystemPrompt === "function" ? asPromptText(ctx.getSystemPrompt()) : "";
