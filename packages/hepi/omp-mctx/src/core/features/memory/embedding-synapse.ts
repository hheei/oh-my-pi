...
export class SynapseEmbeddingError extends Error {
	...
	constructor(
		...
		message: string,
		options?:
			| { retryAfterMs?: number | undefined; permanent?: boolean | undefined; cause?: unknown }
			| undefined,
	) {
...
function extractCatalogEntries(value: unknown): SynapseCatalogEntry[] {
	...
	return raw.flatMap((entry) => {
		...
		const tableEpoch =
			typeof entryEpoch === "number" && Number.isInteger(entryEpoch) ? entryEpoch : envelopeEpoch;
...
export class SynapseEmbeddingProvider implements EmbeddingProvider {
	...
	private async callWithRetry<T>(
		...
		let attempt = 0;
		for (;;) {
			if (signal?.aborted) throw new SynapseEmbeddingError("transport", "Synapse request aborted");
			try {
				if (!this.client)
					throw new SynapseEmbeddingError("transport", "Synapse client is unavailable");
				return await this.client.call<T>(this.options.moduleId ?? "synapse", method, params, {
					timeoutMs,
	...
	private logCallFailure(error: unknown, operation: string): void {
		...
		const suffix =
			classified.retryAfterMs === undefined ? "" : ` retry_after_ms=${classified.retryAfterMs}`;
		log(
