...
export class OpenAICompatibleEmbeddingProvider implements EmbeddingProvider {
		...
		try {
			...
			if (this.model && servedModel && !embeddingModelsMatch(servedModel, this.model)) {
				if (!this.modelMismatchLogged) {
					log(
						`[magic-context] embedding endpoint served a DIFFERENT model than requested — refusing the substituted vectors (they have the wrong dimensions/space). requested="${sanitizeDiagnosticText(this.model)}" served="${sanitizeDiagnosticText(servedModel)}". The endpoint likely substituted a loaded model; load/select "${sanitizeDiagnosticText(this.model)}" on the endpoint, or set embedding.model to the served model.`,
					);
			...
			// endpoint is up but not actually embedding. A mixed payload (some
			// items have vectors, some lack data[].embedding) is also an
			// envelope failure: callers persist the complete items and must
			// still see lastFailureReason for the missing ones.
			if (results.every((r) => r === null)) {
				...
				const failure = this.failure(
					"invalid_envelope",
					`response had keys [${responseKeys.join(", ")}] but data[].embedding was absent for some inputs`,
					true,
				);
				log(`[magic-context] openai-compatible embedding request failed: ${failure.reason}`);
				this.lastFailureReason = failure;
				this.recordSuccess({ preserveFailure: true });
			} else {
				this.recordSuccess();
			}
	...
	private recordSuccess(options?: { preserveFailure?: boolean }): void {
		...
		if (!options?.preserveFailure) this.lastFailureReason = null;
	}
