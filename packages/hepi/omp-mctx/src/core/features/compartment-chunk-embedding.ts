...
function estimateDecodedSearchPoolBytes(rows: readonly StoredCompartmentChunkEmbedding[]): number {
	...
	for (const row of rows) {
		...
		bytes +=
			...
			256 +
			2 * (row.sessionId.length + row.title.length + row.chunkHash.length + row.modelId.length);
	}
	return bytes;
}
...
function cacheDecodedSearchPool(
	...
	while (decodedSearchPoolBytes > DECODED_SEARCH_POOL_CACHE_MAX_BYTES) {
		const oldest = decodedSearchPoolLru.keys().next().value as DecodedSearchPoolEntry | undefined;
		if (!oldest) break;
...
export function getExistingChunkHashes(
	...
	return new Map(
		rows
			.filter((row) => typeof row.windowIndex === "number" && typeof row.chunkHash === "string")
