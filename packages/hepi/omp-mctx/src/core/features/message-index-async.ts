...
async function reconcileSessionIndex(
	...
	await runWithSessionLock(sessionId, async () => {
		...
		let finalWatermark: number;
		if (readMessages.getCount) {
			finalWatermark = readMessages.getCount(sessionId);
		} else {
			fallbackSnapshot = readMessages(sessionId);
			finalWatermark = fallbackSnapshot.length;
		}
		...
		while (cursor < finalWatermark) {
			...
			const messages = readMessages.readPage
				? readMessages.readPage(sessionId, cursor, RECONCILIATION_BATCH_SIZE, finalWatermark)
...
export function scheduleIncrementalIndex(
	...
	const timer = setTimeout(() => {
		...
		void runWithSessionLock(sessionId, () => {
			const message =
				typeof messageSource === "function" ? messageSource(sessionId, messageId) : messageSource;
			if (!message) return;
