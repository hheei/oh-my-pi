...
import { type Database, type Statement, withPrivilegedWriter } from "../shared/sqlite";
...
export async function reconcileAuthorityProject(args: {
	...
	for (const domain of AUTHORITY_DOMAINS) {
		...
		const identity = args.db
			.prepare("SELECT 1 FROM mirror_identity WHERE domain = ? AND module_project = ? LIMIT 1")
		...
		withPrivilegedWriter(args.db, () => {
			args.db
				.transaction(() => {
					args.db
						.prepare("DELETE FROM mirror_identity WHERE domain = ? AND module_project = ?")
...
export interface PrepareAuthorityArgs {
	...
	checksum?:
		| ((domain: AuthorityDomain, rows: readonly Record<string, unknown>[]) => string)
		| undefined;
}
...
function prepareMirrorPageStatements(db: Database): MirrorPageStatements {
	return {
		...
		updateSuperseded: db.prepare("UPDATE memories SET superseded_by_memory_id = ? WHERE id = ?"),
...
function applyMemoryRow(db: Database, feed: ChangefeedRow, statements: MirrorPageStatements): void {
	...
	statements.updateMemory.run(
		...
		has("normalized_hash") ? rowString(row, "normalized_hash") : (existing?.normalized_hash ?? ""),
		has("importance")
			? typeof row.importance === "number" && Number.isFinite(row.importance)
				? row.importance
				: null
		...
		has("retrieval_count") ? rowNumber(row, "retrieval_count") : (existing?.retrieval_count ?? 0),
	...
	withPrivilegedWriter(db, () => {
		db.transaction(() => {
			...
			let resnapshotStatus =
				page.domain === "memories" ? (currentResnapshotState?.status ?? "complete") : "complete";
			if (
				page.domain === "memories" &&
				durableCursor === 0 &&
			...
			withPrivilegedWriter(args.db, () => {
				args.db
					.transaction(() => {
						...
						if (completed) {
							args.db
								.prepare("DELETE FROM mirror_live_staging WHERE generation IS NOT ?")
