...
export function parseCompilerOutput(output: string | null): CompilerResponse {
	...
	if (!parsed.manifest || typeof parsed.manifest !== "object") throw new Error("manifest missing");
...
export function manifestAdvisoryWarnings(code: string, manifest: SmartNoteCheckManifest): string[] {
	...
	for (const file of readFiles) {
		if (!manifest.readFiles?.includes(file)) warnings.push(`manifest omits readFile path ${file}`);
	}
