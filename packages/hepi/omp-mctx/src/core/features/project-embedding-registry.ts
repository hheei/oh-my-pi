...
async function embedCandidateChunkBatch(
	...
	while (i < prepared.length) {
		...
		// Collect classified failure only AFTER this slice's persist pass.
		// Doing it before matching vectors would see persistedIds empty and
		// treat a full success as a failure. Mixed payloads persist complete
		// items and still need the provider's lastFailureReason for the rest.
		if (!signal?.aborted) {
