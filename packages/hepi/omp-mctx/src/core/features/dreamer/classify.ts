...
function isRustAuthorityDrainingError(error: unknown): boolean {
	...
	return (
		...
		(typeof record.message === "string" &&
			/rust memory authority.*not ready|authority.*draining/i.test(record.message))
	);
}
...
export async function runClassify(args: ClassifyArgs): Promise<ClassifyResult> {
	...
	try {
		for (let i = 0; i < chunks.length; i += 1) {
			...
			if (!chunk) break;
			const counts = await classifyOneChunk(args, chunk, anchors, sliceMs, abortController.signal);
...
async function runClassifyThroughModule(
	...
	const rows = parsed.map((entry) => {
		...
		return {
			...
			shareable:
				entry.shareable === true && hasShareabilitySensitiveText(candidate.contextMemory.content)
					? false
					: entry.shareable,
		};
	});
	...
	for (const row of rejectedRows) {
		const reason =
			row && typeof row === "object" && typeof (row as { reason?: unknown }).reason === "string"
...
export function applyClassifications(
	...
	runLeaseGuardedWrite(args.db, args.holderId, args.leaseKey, () => {
		for (const p of parsed) {
			...
			const shareable =
				p.shareable === true && hasShareabilitySensitiveText(memory.content) ? false : p.shareable;
