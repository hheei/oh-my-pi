 ...
 *   - Node → uses `node:sqlite` (`DatabaseSync`, Node 22.5+).
 *
 * Bun has no `node:sqlite`, and Node has no `bun:sqlite`. Static
 ...
 * module requiring per-ABI prebuilds. `node:sqlite` is built into the runtime.
 *
...
function buildNodeSqliteDatabaseClass(DatabaseSync: any): typeof BetterSqlite3 {
	...
	class NodeSqliteDatabase extends DatabaseSync {
		...
		// (e.g. `.run([x, y])`) silently work on bun:sqlite yet break node:sqlite.
		// Wrapping every prepared statement here keeps the two backends' bind
		// surface truly identical so this whole class is impossible regardless of
		// how a call site writes its bind. Named-object binds (`.run({k:v})`), no-arg
