...
export function resolveLkgModelKeys(messages: MessageLike[]): LkgModelKeys {
	for (let index = messages.length - 1; index >= 0; index -= 1) {
		...
		if (info?.role === "assistant" && typeof provider === "string" && typeof model === "string") {
			return { modelKey: `${provider}/${model}`, providerKey: provider };
		}
	}
...
export function projectLkgEntry(messages: MessageLike[]): LkgEntryProjection[] {
	return messages.map((message) => {
		...
		const timeRecord = time && typeof time === "object" ? (time as Record<string, unknown>) : null;
		...
		for (const rawPart of messageParts(message)) {
			...
			const status =
				state && typeof state === "object" ? (state as Record<string, unknown>).status : undefined;
			if (status !== "completed") {
...
function hasSyntheticParts(message: MessageLike): boolean {
	...
	return (
		...
		parts.every((part) => {
			return Boolean(
				part && typeof part === "object" && (part as Record<string, unknown>).synthetic === true,
			);
		})
	);
}
