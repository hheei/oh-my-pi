...
export function buildChannel1Reminder(
	...
	switch (level) {
		case "gentle":
			body =
				`Housekeeping, not a crisis. ~${amount} tokens of tool output have not been reduced. ` +
				`When you are done with earlier outputs, dropping them with ctx_reduce keeps context lean.`;
			break;
		case "firm":
			body =
				`~${amount} tokens of unreduced tool output have built up. This is housekeeping, not a crisis. ` +
		...
		case "urgent":
			body =
				...
				`This is still housekeeping. Consider dropping spent outputs with ctx_reduce so the archived span is the part that matters.`;
			break;
	}
	return `\n\n<system-reminder>\n${body}${hintText}\n</system-reminder>`;
}
