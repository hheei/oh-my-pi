...
function buildProtectedTailBoundary(ctx: ResolvedBoundaryContext): ProtectedTailBoundarySnapshot {
...
export function validateBoundarySnapshot(args: {
	...
	if (snapshot.eligibleEndOrdinal > snapshot.offset) {
		checks.push([snapshot.eligibleEndOrdinal - 1, snapshot.eligibleEndMessageId, "eligibleEnd"]);
	}
