...
export function createSystemPromptHashHandler(deps: {
	...
	/** Whether ctx_search is registered; defaults to true for legacy callers. */
	searchEnabled?: boolean | undefined;
	/** Whether ctx_note is registered; defaults to true for legacy callers. */
	...
	const handler = async (
		...
		if (
			...
			const guidance = buildMagicContextSection(
				...
				deps.searchEnabled !== false,
				deps.noteEnabled !== false,
			);
