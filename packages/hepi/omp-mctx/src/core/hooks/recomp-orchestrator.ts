...
import { clearRecompStaging, getCompartments } from "../features/compartment-storage";
import { isMemoryMigrationDone, runMemoryMigration } from "../features/memory/memory-migration";
import { resolveProjectIdentity } from "../features/memory/project-identity";
import { clearEmergencyRecovery, isWrapupInProgress } from "../features/storage-meta-persisted";
...
export function contextualizeUpgradeReason(reason: string): string {
	...
	if (/missing the tiered paraphrase structure \(p1\.\.p4\)/i.test(rewritten)) {
		return `Your configured \`historian.model\` could not produce the required tiered (p1..p4) compartment output. Choose a historian model that can follow the XML format in \`/ext-settings\`, then run \`/ctx-session-upgrade\` again. No compartments were rewritten. Validation error: ${rewritten}`;
	}
	return rewritten;
}
