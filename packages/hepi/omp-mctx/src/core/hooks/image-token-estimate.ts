...
function parseJpegDimensions(b: Uint8Array): { w: number; h: number } | null {
	...
	while (i < b.length - 8) {
		...
		if (isSofMarker(marker)) {
			...
			const h = (u8(b, i + 5) << 8) | u8(b, i + 6);
			const w = (u8(b, i + 7) << 8) | u8(b, i + 8);
			if (w && h) return { w, h };
			return null;
		}
		...
		const segLen = (u8(b, i + 2) << 8) | u8(b, i + 3);
...
function parseWebpDimensions(b: Uint8Array): { w: number; h: number } | null {
	...
	const variant = String.fromCharCode(u8(b, 12), u8(b, 13), u8(b, 14), u8(b, 15));
	if (variant === "VP8 ") {
		...
		const w = (u8(b, 26) | (u8(b, 27) << 8)) & 0x3fff;
		const h = (u8(b, 28) | (u8(b, 29) << 8)) & 0x3fff;
		if (w && h) return { w, h };
		...
		const w = 1 + (u8(b, 24) | (u8(b, 25) << 8) | (u8(b, 26) << 16));
		const h = 1 + (u8(b, 27) | (u8(b, 28) << 8) | (u8(b, 29) << 16));
		if (w && h) return { w, h };
	}
	return null;
}
...
function parseGifDimensions(b: Uint8Array): { w: number; h: number } | null {
	...
	const w = u8(b, 6) | (u8(b, 7) << 8);
	const h = u8(b, 8) | (u8(b, 9) << 8);
	if (!w || !h) return null;
	return { w, h };
}
