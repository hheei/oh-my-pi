...
export async function runPiHistorian(deps: PiHistorianDeps): Promise<void> {
 ...
 // Capture before asynchronous historian work. The publish transaction checks
 // this again, so native compaction may proceed without waiting for us.
  ...
  await withRawMessageProvider(sessionId, provider, async () => {
   ...
   try {
    ...
    if (agentMemory?.enabled && !skipUnanchoredPromotion && agentMemory.project) {
     ...
     if (remoteSessionId)
      for (const fact of validatedPass.facts ?? []) {
       ...
       if (!source) continue;
       const tainted = agentMemoryTaint?.isHostEntryTainted?.(source.messageId) === true;
    ...
    if (firstKeptEntryId && lastNewEndMessageId) {
     ...
     if (!staged) sessionLog(sessionId, "historian marker staging rejected by native compaction fence");
    }
    db.exec("COMMIT");
    published = true;
   } finally {
...
export function buildPiCompactionSummary(
 ...
 return `Magic Context compacted ${titles.length} segments: ${shown}; …and ${titles.length - MAX_SUMMARY_TITLES
  } more`;
}
