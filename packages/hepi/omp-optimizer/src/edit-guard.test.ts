...
function makeGuard(activeTools: readonly string[] = ["edit"]) {
	const handlers = new Map<
		string,
		(event: unknown, context?: { abort(): void }) => unknown
	>();
	...
	const pi = {
		...
		on(
			event: string,
			handler: (event: unknown, context?: { abort(): void }) => unknown,
		) {
	...
	editGuard(
		...
		{ set: () => {} } as unknown as OptimizerStatus,
	);
...
describe("edit guard detection", () => {
	...
	it("matches shell apply_patch commands but not plain mentions", () => {
		...
		expect(hasStreamingApplyPatchCommand("please do not use apply_patch")).toBe(
			false,
		);
	});
	...
	it("ignores valid hashline bodies and other edit formats", () => {
		...
		expect(hasMalformedHashlineInput("please avoid @@ when editing")).toBe(
			false,
		);
	});
});
...
describe("edit guard runtime", () => {
	it("aborts malformed hashline once, then sends retry guidance", () => {
		...
		const update = handlers.get("message_update") as (
			event: unknown,
			context: { abort(): void },
		) => void;
		const settled = handlers.get("agent_end") as (
		...
		callUpdate(
			update,
			"edit",
			{ input: `${historicalMalformedPayload}\n@@` },
			() => {
				aborts += 1;
			},
		);
